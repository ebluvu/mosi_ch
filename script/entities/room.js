let Room = {
    ...
    rename: (that, roomIndex, newName) => {
        ...
        if (newName === '') {
            that.setState({ showErrorOverlay: true, errorMessage: `房間名稱不能為空!` })
        } else if (roomList.find(r => r.name === newName)) {
            that.setState({ showErrorOverlay: true, errorMessage: `其他房間已經命名為 "${newName}"!` })
        } else {
            room.name = newName
    ...
    import: (that, roomIndex, roomData) => {
        try {
            ...
            console.error('無法導入房間！', e)
            that.setState({
                showErrorOverlay: true,
                errorMessage: isStr(e) ? e : '無法導入房間！'
            })
        }
    },
