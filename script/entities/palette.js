    ...
    import: (that, paletteData) => {
        try {
            ...
            if (palette.colorList.length < 2) {
                throw('這個調色盤的顏色太少了，你至少需要2種')
            }

            Palette.add(that, palette)
        }
        catch (e) {
            console.error('無法導入調色板！', e)
            that.setState({ showErrorOverlay: true, errorMessage: '無法導入調色板！' })
        }
    },
    ...
    rename:  (that, paletteIndex, newName) => {
        ...
        if (newName === '') {
            that.setState({
                ...
                errorMessage: `配色名稱不能為空!`
            })
        } else if (paletteList.find(p => p.name === newName)) {
            that.setState({
                showErrorOverlay: true,
                errorMessage: `其他配色已經命名為 "${newName}"!`
            })
        } else {
