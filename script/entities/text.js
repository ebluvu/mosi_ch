...
return {
    ...
    drawBackground: (context, fontData, position, linesPerPage = 2, bgColor = 'black', world = null, palette = null) => {
        if (world && world.textboxSkin) {
            ...
            if (pos === 'top') {
                ...
                // 四角
                drawSkinBlock(context, skin.fillList[0], x0, y0, fw, fh, palette)
                drawSkinBlock(context, skin.fillList[2], x2, y0, fw, fh, palette)
                drawSkinBlock(context, skin.fillList[6], x0, y2, fw, fh, palette)
                drawSkinBlock(context, skin.fillList[8], x2, y2, fw, fh, palette)
                ...
                for (let xx = x1; xx < x2; xx += fw) {
                    ...
                    if (w < fw) {
                        ...
                        drawSkinBlock(context, sliced, xx, y0, w, fh, palette)
                    } else {
                        drawSkinBlock(context, skin.fillList[1], xx, y0, fw, fh, palette)
                    }
                    // 下邊線
                    if (w < fw) {
                        ...
                        drawSkinBlock(context, sliced, xx, y2, w, fh, palette)
                    } else {
                        drawSkinBlock(context, skin.fillList[7], xx, y2, fw, fh, palette)
                    }
                }
                // 左右邊
                for (let yy = y1; yy < y2; yy += fh) {
                    let h = Math.min(fh, y2 - yy)
                    drawSkinBlock(context, skin.fillList[3], x0, yy, fw, h, palette)
                    drawSkinBlock(context, skin.fillList[5], x2, yy, fw, h, palette)
                }
                // 中間
                for (let xx = x1; xx < x2; xx += fw) {
                    let w = Math.min(fw, x2 - xx)
                    for (let yy = y1; yy < y2; yy += fh) {
                        let h = Math.min(fh, y2 - yy)
                        drawSkinBlock(context, skin.fillList[4], xx, yy, w, h, palette)
                    }
                }
    ...
    drawContinueIndicator: (context, fontData, bgX, bgY, bgWidth, bgHeight, indicatorColor = 'white', world = null, palette = null, frameIndex = null) => {
        if (world && world.textboxSkin) {
            ...
            let indicatorWidth = Math.floor(fw * indicatorMultiplier)
            let indicatorHeight = Math.floor(fh * indicatorMultiplier)
            ...
            drawSkinBlock(context, indicator, indicatorX, indicatorY, indicatorWidth, indicatorHeight, palette)
            return
        }
        let textScale = window.textScale || 2
        // 對話箭頭大小：x1 = 0.5, x2 = 1
...
// 輔助函式：將像素陣列畫到指定位置
function drawSkinBlock(context, pixelData, x, y, w, h, palette) {
    for (let cy = 0; cy < h; cy++) {
        for (let cx = 0; cx < w; cx++) {
            let idx = cy * w + cx
            let val = pixelData[idx]
            if (val && palette && palette.colorList[val]) {
